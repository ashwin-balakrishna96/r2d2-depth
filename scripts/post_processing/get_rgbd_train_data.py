...
def get_camera_extrinsics(serial_numbers, traj, traj_idxs):
    extrinsics_left_keys = [str(serial_num) + "_left" for serial_num in serial_numbers]
    extrinsics_right_keys = [str(serial_num) + "_right" for serial_num in serial_numbers]
    try:
        combined_extrinsics = np.array([[[frame["observation"]["camera_extrinsics"][extrinsics_left_key], frame["observation"]["camera_extrinsics"][extrinsics_right_key]] \
                                            for i, frame in enumerate(traj) \
                                            if i in traj_idxs] for extrinsics_left_key, extrinsics_right_key in zip(extrinsics_left_keys, extrinsics_right_keys)])
...
def get_input_output_paths(r2d2_data_path, save_path, prefix):
    folder_depth = 5# TODO(Ashwin): don't hardcode this
...
def main(process_id, num_processes):
    ...
    if os.path.exists(os.path.join(SAVE_PATH, 'failures.pkl')):
        ...
        failures = {'Missing SVO/H5 File': set(), 'Trajectory Loading Error': set(), 'Trajectory too Long': set(),
                        'Trajectory is Empty': set(), 'Shape Mismatch': set(), 'Missing Extrinsics': set()}
    ...
    for i, (input_traj_path, output_traj_path) in enumerate(zip(input_traj_paths, output_traj_paths)):
        if i % num_processes == process_id:
            ...
            if combined_extrinsics is None:
                print("SKIPPED: missing extrinsics")
                failures['Missing Extrinsics'].add(traj_name)
                continue
            ...
            for camera_idx in range(left_rgb_im_traj.shape[1]):
                ...
                for t in range(left_rgb_im_traj.shape[0]):
                    ...
                    save_depth = np.clip(1000 * cv2.resize(tri_depth_im_traj[t, camera_idx], resize_shape), 0, 65535).astype('uint16')
                    np.save(os.path.join(depth_dir, f"tri_depth_{t:03}.npy"), save_depth)
